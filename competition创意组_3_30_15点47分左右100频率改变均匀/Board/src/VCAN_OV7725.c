...
#include "common.h"
#include "include.h"
#include "VCAN_ImageDeal.h"

#define OV7725_EAGLE_Delay_ms(time)  DELAY_MS(time)

uint8   a_In_imgbuff[OV7725_EAGLE_H][OV7725_EAGLE_W];                              //定义存储接收图像的数组
uint8   *ov7725_eagle_img_buff;
uint8 imgbuff[CAMERA_SIZE];                             //定义存储接收图像的数组

YesNo_t imgDealOk=Yes;
...
void ov7725_eagle_conversion(void)
{
    uint16_t i=0,j=0;
 //   num = 0;
    uint8_t dat,mask = 0x80;
    uint8_t *psource = imgbuff;
    uint8_t *pdest=    &a_In_imgbuff[0][0];   
    
    for(i = 0;i < OV7725_EAGLE_SIZE;i++)
    {
        dat = *psource;
        for(j = 0;j < 8;j++)
        {
           if(mask & dat)  *pdest = 0XFF;
           else            *pdest = 0;
           mask>>=1;
           pdest++;     
        }
